...
export default function PaisCard({name}: { name: string }){
    ...
    <div className='flex flex-col'>
        <Header/>
        <div className='flex flex-col h-[80vh] gap-[40px] justify-center text-[16px]'>
            <Link href={'/'} className='mx-[13vw] dark:text-shadow-black dark:hover:translate-y-[-1px] '>↩︎ Voltar</Link>
            <div className='flex flex-col items-center  justify-center'>
                {paisFiltrado && (
                    <div key={paisFiltrado.name} className='flex flex-col  justify-center md:flex-row  md:items-center gap-[40px] w-[90vw] rounded-[5px]'>
                        ...
                        <div className='flex flex-col'>
                            <h1 className='dark:text-shadow-lg/15 text-shadow-lg/15 px-[10px] font-black text-[24px]'>{paisFiltrado.name}</h1>
                                ...
                                <ul className='flex flex-wrap p-[10px]'>
                                    {paisFiltrado.borders ? paisFiltrado.borders.map((b: string) => {
                                        ...
                                        return (
                                            ...
                                            <li className='flex items-center dark:hover:translate-y-[-1px] dark:border-[var(--foreground)] p-[3px] w-max m-[3px] font-light border-[1px]  border-gray-300   rounded-[5px] text-[12px] px-[10px]'>{paisFrontal?.name}</li></Link>
                                        )
                                    }) : <Link href={`/`}><li className='flex items-center dark:hover:translate-y-[-1px] w-max p-[3px] px-[10px] m-[3px] text-[12px] font-light border border-gray-300 rounded-[5px]'>Não faz fronteira com nenhum outro pais.</li></Link> }
                                    </ul>
                            </div>
                        </div>
                    </div>)}
