    ...
    return(
        <header>
            <div className="flex justify-between items-center dark:bg-[var(--cards)]  dark:border-none dark:shadow-[0px_10px_10px_rgba(0,0,0,1)] h-[80px] p-[40px] bg-[#ece6e1] text-black text-[16px]  border-b-[1px] border-[#D9D9D9]  shadow-md">
                ...
                <button onClick={() => setTheme(theme ==='light' ? 'dark' : 'light')} className="dark:text-[#BFB2A3] hover:translate-y-[-1px] hover:text-shadow-lg transition-all duration-150 text-shadow-md font-light text-[#594834] cursor-pointer w-max h-max ">{theme === 'dark' ? '☼ Light Mode' : '☽ Dark mode'}</button>
            </div>
        </header>
    )
}